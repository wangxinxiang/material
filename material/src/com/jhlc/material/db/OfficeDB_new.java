 ...
 */
public class OfficeDB_new {
    private OfficeDBHelper_new helper;
    public OfficeDB_new(Context context){
        helper = OfficeDBHelper_new.getHelper(context);
    }
    private SQLiteDatabase db;
    private StringBuffer sql_insert;
    private SQLiteStatement sm;

    public void addOffce(String officename,String username,String headimage,String job,String area){
        db = helper.getReadableDatabase();
        ContentValues cv = new ContentValues();
        baseCV(officename, username, headimage, job,area, cv);
...
//        db.close();
    ...
    private void baseCV(String officename, String username, String headimage, String job,String area, ContentValues cv) {
...
//        db.close();
    ...
    //数据更新操作，当人物的职位,头像,所属地区,发生变化时更新
    public void updateDataByUsername(String username,String headimage,String job,String area,SQLiteDatabase db){

        String sql = "update officeuser set headimage=?,job=?,area=? where username=?";
        db.execSQL(sql, new Object[]{headimage, job,area, username});//执行修改操作
    }
...
//        db.close();
    ...
    //获取某个地区的所有数据
    public List<OfficeBean> getAreaData(String area){
         db = helper.getReadableDatabase();
        List<OfficeBean> list = new ArrayList<OfficeBean>();
        String sql = "select * from officeuser where area like ?";
        ...
        while (c.moveToNext()) {
            ...
            bean.setHeadimage(c.getString(c.getColumnIndex("headimage")));
            bean.setJob(c.getString(c.getColumnIndex("job")));
            bean.setArea(c.getString(c.getColumnIndex("area")));
            String officename = c.getString(c.getColumnIndex("officename"));
            //判断list中是否已经有OfficeBean，有就直接将bean加入到OfficeBean的list中,没有就新建一个OfficeBean加入到list中
            boolean f = true;
            for (int i = 0; i< list.size(); i++){
...
//        db.close();
    ...
    public List<OfficeBean> getDataBySearch(String search ,String area){
         db = helper.getReadableDatabase();
        List<OfficeBean> list = new ArrayList<OfficeBean>();
        String sql = "select * from officeuser where (officename like ? or username like ? or job like ?) and area like ?";
        Cursor c = db.rawQuery(sql,new String[]{"%"+search+"%","%"+search+"%","%"+search+"%",area});
        while (c.moveToNext()) {
            OfficeUserBean bean = new OfficeUserBean();
            bean.setUsername(c.getString(c.getColumnIndex("username")));
            bean.setHeadimage(c.getString(c.getColumnIndex("headimage")));
            ...
            for (int i = 0; i < list.size(); i++) {
                ...
                for (int j = 0; j < ob.getList().size(); j++) {
                    ...
                    if(!isDataByUsername(bean.getUsername(),db)){
                        ...
                        ContentValues cv = new ContentValues();
                        baseCV(ob.getOfficename(), bean.getUsername(), bean.getHeadimage(),bean.getJob(),bean.getArea(),cv);
                        db.insert("officeuser",null,cv);
                    }else{
                        //如果该username存在就update更新该username中的数据
                        updateDataByUsername(bean.getUsername(),bean.getHeadimage(),bean.getJob(),bean.getArea(),db);
                    }
                }
            }
//        db.close();
...
//        db.close();
    ...
    //用于数据更新的操作
    public void updateOffice_sd(List<OfficeBean> list){
        // = System.currentTimeMillis()
        db = helper.getWritableDatabase();
        int list_size=list.size();
        for (int i = 0; i < list_size; i++) {
            OfficeBean ob = list.get(i);
            int ob_size=ob.getList().size();
            for (int j = 0; j <ob_size; j++) {
                OfficeUserBean bean = ob.getList().get(j);
                if(!isDataByUsername(bean.getUsername(),db)){
                    //如果该username不存在表中就插入数据
                    sql_insert = new StringBuffer();
                    sql_insert.append("INSERT INTO officeuser(officename,username,headimage,job,area) ");
                    sql_insert.append(" VALUES( ?, ?, ?, ?, ?)");
                    sm=db.compileStatement(sql_insert.toString());
                    sm.bindString(1,ob.getOfficename());
                    sm.bindString(2,bean.getUsername());
                    sm.bindString(3,bean.getHeadimage());
                    sm.bindString(4,bean.getJob());
                    sm.bindString(5, bean.getArea());
                    sm.executeInsert();
                }else{
                    //如果该username存在就update更新该username中的数据
                    updateDataByUsername(bean.getUsername(),bean.getHeadimage(),bean.getJob(),bean.getArea(),db);
